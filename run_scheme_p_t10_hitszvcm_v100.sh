...
PART_LR_MULT="${PART_LR_MULT:-10.0}"
...
DESC="${DESC:-SchemeP_gated_residual_fromDv1_t10_hitszvcm_v100_bs$((P_NUM * K_NUM))}"
...
python train_m3reid.py \
  ...
  --use_m3plus \
  --m3plus_mode gated_residual_fusion \
  ...
  --gpu "${GPU}" \
  "${EXTRA_ARGS[@]}" \
  2>&1 | tee "${LOG_DIR}/scheme_p_t10_hitszvcm_v100_$(date +%Y%m%d_%H%M%S).log"
