...
PART_TRIPLET_WEIGHT="${PART_TRIPLET_WEIGHT:-0.015}"
PART_MMA_WEIGHT="${PART_MMA_WEIGHT:-0.001}"
ROUTER_WEIGHT="${ROUTER_WEIGHT:-0.03}"
MODALITY_ADV_WEIGHT="${MODALITY_ADV_WEIGHT:-0.01}"
INVARIANT_CONSISTENCY_WEIGHT="${INVARIANT_CONSISTENCY_WEIGHT:-0.05}"
...
DESC="${DESC:-SchemeUNoBoT_invariant_specific_t10_buptcampus_v100_bs$((P_NUM * K_NUM))_from_nobot}"
...
python train_m3reid.py \
  ...
  --use_m3plus \
  --m3plus_mode invariant_specific_calibration \
  ...
  --modality_adv_weight "${MODALITY_ADV_WEIGHT}" \
  --invariant_consistency_weight "${INVARIANT_CONSISTENCY_WEIGHT}" \
  ...
  --gpu "${GPU}" \
  "${EXTRA_ARGS[@]}" \
  2>&1 | tee "${LOG_DIR}/scheme_u_t10_buptcampus_v100_$(date +%Y%m%d_%H%M%S).log"
