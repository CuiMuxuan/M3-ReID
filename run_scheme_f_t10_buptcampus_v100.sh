...
if [ -z "${BASELINE_CKPT:-}" ]; then
  echo "BASELINE_CKPT is required, e.g. BASELINE_CKPT=/root/work/M3-ReID/ckptlog/BUPTCampus/.../modelckpt/model_best.pth"
  exit 1
fi
...
PROTO_WEIGHT="${PROTO_WEIGHT:-0.15}"
PART_PROTO_WEIGHT="${PART_PROTO_WEIGHT:-0.05}"
...
DESC="${DESC:-SchemeF_proto_dualfusion_t10_buptcampus_v100_bs$((P_NUM * K_NUM))}"
...
python train_m3reid.py \
  ...
  --gpu "${GPU}" \
  "${EXTRA_ARGS[@]}" \
  2>&1 | tee "${LOG_DIR}/scheme_f_t10_buptcampus_v100_$(date +%Y%m%d_%H%M%S).log"
