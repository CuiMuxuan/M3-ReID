...
DESC="${DESC:-SchemeAENoBoT_reliability_quad_fromAA_fa${FUSION_ALPHA}_pa${PROJECTION_ALPHA}_ca${CALIBRATION_ALPHA}_bgw${BRANCH_GATE_WEIGHT}_t10_buptcampus_v100_bs$((P_NUM * K_NUM))}"
...
python train_m3reid.py \
  ...
  --gpu "${GPU}" \
  "${EXTRA_ARGS[@]}" \
  2>&1 | tee "${LOG_DIR}/scheme_ae_t10_buptcampus_v100_$(date +%Y%m%d_%H%M%S).log"
