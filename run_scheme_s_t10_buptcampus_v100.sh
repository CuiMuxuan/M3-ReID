...
DESC="${DESC:-SchemeSNoBoT_reliability_partfusion_t10_buptcampus_v100_bs$((P_NUM * K_NUM))_from_nobot}"
...
python train_m3reid.py \
  ...
  --use_m3plus \
  --m3plus_mode reliability_part_fusion \
  ...
  --gpu "${GPU}" \
  "${EXTRA_ARGS[@]}" \
  2>&1 | tee "${LOG_DIR}/scheme_s_t10_buptcampus_v100_$(date +%Y%m%d_%H%M%S).log"
