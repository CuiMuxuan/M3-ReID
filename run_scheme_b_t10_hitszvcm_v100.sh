...
if [ -z "${BASELINE_CKPT:-}" ]; then
  echo "BASELINE_CKPT is required, e.g. BASELINE_CKPT=/root/work/M3-ReID/ckptlog/HITSZVCM/.../modelckpt/model_best.pth"
  exit 1
fi
...
python train_m3reid.py \
  ...
  --gpu "${GPU}" \
  "${EXTRA_ARGS[@]}" \
  2>&1 | tee "${LOG_DIR}/scheme_b_t10_hitszvcm_v100_$(date +%Y%m%d_%H%M%S).log"
