...
PART_ID_WEIGHT="${PART_ID_WEIGHT:-0.10}"
PART_TRIPLET_WEIGHT="${PART_TRIPLET_WEIGHT:-0.05}"
PART_MMA_WEIGHT="${PART_MMA_WEIGHT:-0.005}"
FUSION_ID_WEIGHT="${FUSION_ID_WEIGHT:-0.20}"
FUSION_TRIPLET_WEIGHT="${FUSION_TRIPLET_WEIGHT:-0.12}"
...
DESC="${DESC:-SchemeO_supervised_dualfusion_fromDv1_alpha${FUSION_ALPHA}_t10_hitszvcm_v100_bs$((P_NUM * K_NUM))}"
...
python train_m3reid.py \
  ...
  --use_m3plus \
  --m3plus_mode supervised_dual_fusion \
  ...
  --gpu "${GPU}" \
  "${EXTRA_ARGS[@]}" \
  2>&1 | tee "${LOG_DIR}/scheme_o_t10_hitszvcm_v100_$(date +%Y%m%d_%H%M%S).log"
