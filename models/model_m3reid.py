...
class M3ReID(nn.Module):
    ...
    def __init__(self, sample_seq_num, class_num, use_enhancements=False, m3plus_mode='full', part_num=4,
        ...
        if self.use_part_branch:
            ...
            if self.use_local_residual:
                ...
                calibration_max_scale = 0.08 if use_quad_calibration_scale else 0.06
                if self.use_reliability_quad_calibrated_fusion:
                    self.reliability_quad_gate = ReliabilityBalancedQuadGate(
                ...
                if (
                    ...
                    if self.use_adaptive_projection_calibrated_fusion or self.use_adaptive_quad_calibrated_fusion:
                        init_projection_alpha = (
                            projection_alpha if self.use_adaptive_quad_calibrated_fusion else fusion_alpha
                        )
    ...
    def _adaptive_quad_calibrated_eval(self, global_embed, part_embed, projection_embed, calibration_embed):
        part_alpha = min(max(self.fusion_alpha, 0.0), 1.0)
        projection_alpha, calibration_alpha = self.adaptive_projection_calibration_gate(
            global_embed, projection_embed, calibration_embed
        )
        branch_budget = max(1.0 - part_alpha, 1e-6)
        branch_sum = projection_alpha + calibration_alpha
        scale = (branch_budget / branch_sum.clamp_min(1e-6)).clamp(max=1.0)
        ...
        calibration_eval = self.l2_norm(calibration_embed) * torch.sqrt(calibration_alpha.clamp_min(1e-6))
        return (
            torch.cat([global_eval, part_eval, projection_eval, calibration_eval], dim=1),
            projection_alpha,
            calibration_alpha,
        )
    ...
    def forward(self, inputs):
        ...
        if self.use_part_branch:
            ...
            if self.use_local_residual:
                ...
                if self.use_direction_aware_quad_calibrated_fusion:
                    calibration_pool, router_logits, calibration_gate = self._checkpoint_if_enabled(
                        self.direction_aware_calibration, x_pool, part_pool
                    )
                else:
                    calibration_pool, calibration_gate = self._checkpoint_if_enabled(
                        self.dual_calibration_fusion, x_pool, part_pool
        ...
        if self.training:
            ...
            if self.use_part_supervision:
                ...
                if (
                    (
                        self.use_bidirectional_calibration
                        or self.use_invariant_specific_calibration
                        or self.use_direction_aware_quad_calibrated_fusion
                    )
                ...
                if (
                    ...
                    if self.use_adaptive_projection_calibrated_fusion or self.use_adaptive_quad_calibrated_fusion:
                        if self.use_adaptive_quad_calibrated_fusion:
                            adaptive_eval, projection_alpha, calibration_alpha = self._adaptive_quad_calibrated_eval(
                    ...
                    if self.use_reliability_quad_calibrated_fusion:
                        reliability_eval, part_alpha, projection_alpha, calibration_alpha = self.reliability_quad_gate(
                            x_embed_mean, part_embed_mean, aux['projection_embed_mean'], calibration_embed_mean
                        )
                        aux['fusion_embed_mean'] = reliability_eval
                        reliability_bn = self.adaptive_projection_bn_neck(reliability_eval)
                        aux['fusion_logits_mean'] = self.adaptive_projection_classifier(
                            self.feature_dropout(reliability_bn)
                        )
                    ...
                    if self.use_anchor_sparse_quad_fusion:
                        (
                            ...
                            calibration_alpha,
                            residual_gate,
                        ) = self.anchor_sparse_quad_mixture(
                            x_embed_mean, part_embed_mean, aux['projection_embed_mean'], calibration_embed_mean
                        )
                        aux['fusion_embed_mean'] = anchor_sparse_eval
                        anchor_sparse_bn = self.adaptive_projection_bn_neck(anchor_sparse_eval)
                        aux['fusion_logits_mean'] = self.adaptive_projection_classifier(
                            self.feature_dropout(anchor_sparse_bn)
                        )
                        aux['branch_budget'] = branch_budget
                        aux['part_alpha'] = part_alpha
                        aux['projection_alpha'] = projection_alpha
            ...
            if (
                ...
                calibration_embed = calibration_embed.reshape(-1, t, calibration_embed.shape[-1]).mean(dim=1)
                if self.use_adaptive_quad_calibrated_fusion:
                    return self._adaptive_quad_calibrated_eval(
