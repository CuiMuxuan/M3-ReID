...
DESC="${DESC:-SchemeAINoBoT_anchor_sparse_quad_fromD_pa${PROJECTION_ALPHA}_fa${FUSION_ALPHA}_ca${CALIBRATION_ALPHA}_t10_hitszvcm_v100_bs$((P_NUM * K_NUM))}"
...
python train_m3reid.py \
  ...
  --use_m3plus \
  --m3plus_mode anchor_sparse_quad_fusion \
  ...
  --gpu "${GPU}" \
  "${EXTRA_ARGS[@]}" \
  2>&1 | tee "${LOG_DIR}/scheme_ai_t10_hitszvcm_v100_$(date +%Y%m%d_%H%M%S).log"
