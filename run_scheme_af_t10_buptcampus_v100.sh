...
FUSION_ID_WEIGHT="${FUSION_ID_WEIGHT:-0.010}"
...
CALIBRATION_ID_WEIGHT="${CALIBRATION_ID_WEIGHT:-0.020}"
CALIBRATION_TRIPLET_WEIGHT="${CALIBRATION_TRIPLET_WEIGHT:-0.010}"
...
DESC="${DESC:-SchemeAFNoBoT_supervised_quad_fromAA_pa${PROJECTION_ALPHA}_fa${FUSION_ALPHA}_ca${CALIBRATION_ALPHA}_fw${FUSION_ID_WEIGHT}_${FUSION_TRIPLET_WEIGHT}_t10_buptcampus_v100_bs$((P_NUM * K_NUM))}"
...
python train_m3reid.py \
  ...
  --use_m3plus \
  --m3plus_mode supervised_quad_calibrated_fusion \
  ...
  --gpu "${GPU}" \
  "${EXTRA_ARGS[@]}" \
  2>&1 | tee "${LOG_DIR}/scheme_af_t10_buptcampus_v100_$(date +%Y%m%d_%H%M%S).log"
