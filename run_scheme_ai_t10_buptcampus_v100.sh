...
DESC="${DESC:-SchemeAINoBoT_anchor_sparse_quad_fromAA_pa${PROJECTION_ALPHA}_fa${FUSION_ALPHA}_ca${CALIBRATION_ALPHA}_t10_buptcampus_v100_bs$((P_NUM * K_NUM))}"
...
python train_m3reid.py \
  ...
  --gpu "${GPU}" \
  "${EXTRA_ARGS[@]}" \
  2>&1 | tee "${LOG_DIR}/scheme_ai_t10_buptcampus_v100_$(date +%Y%m%d_%H%M%S).log"
