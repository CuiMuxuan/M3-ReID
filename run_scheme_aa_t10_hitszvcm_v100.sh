...
DESC="${DESC:-SchemeAANoBoT_quad_calibrated_fromBaseline_pa${PROJECTION_ALPHA}_fa${FUSION_ALPHA}_ca${CALIBRATION_ALPHA}_t10_hitszvcm_v100_bs$((P_NUM * K_NUM))}"
...
python train_m3reid.py \
  ...
  --gpu "${GPU}" \
  "${EXTRA_ARGS[@]}" \
  2>&1 | tee "${LOG_DIR}/scheme_aa_t10_hitszvcm_v100_$(date +%Y%m%d_%H%M%S).log"
