...
PART_ID_WEIGHT="${PART_ID_WEIGHT:-0.03}"
PART_TRIPLET_WEIGHT="${PART_TRIPLET_WEIGHT:-0.010}"
PART_MMA_WEIGHT="${PART_MMA_WEIGHT:-0.001}"
PROJECTION_ID_WEIGHT="${PROJECTION_ID_WEIGHT:-0.04}"
PROJECTION_TRIPLET_WEIGHT="${PROJECTION_TRIPLET_WEIGHT:-0.020}"
PROJECTION_MMA_WEIGHT="${PROJECTION_MMA_WEIGHT:-0.002}"
PROJECTION_GATE_WEIGHT="${PROJECTION_GATE_WEIGHT:-0.02}"
PROJECTION_GATE_IR_TARGET="${PROJECTION_GATE_IR_TARGET:-0.7}"
PROJECTION_GATE_RGB_TARGET="${PROJECTION_GATE_RGB_TARGET:-0.3}"
...
DESC="${DESC:-SchemeVNoBoT_anchor_projection_alpha${FUSION_ALPHA}_t10_buptcampus_v100_bs$((P_NUM * K_NUM))_from_nobot}"
...
python train_m3reid.py \
  ...
  --use_m3plus \
  --m3plus_mode anchor_projection_fusion \
  ...
  --gpu "${GPU}" \
  "${EXTRA_ARGS[@]}" \
  2>&1 | tee "${LOG_DIR}/scheme_v_t10_buptcampus_v100_$(date +%Y%m%d_%H%M%S).log"
