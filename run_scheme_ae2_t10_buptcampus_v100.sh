...
BASELINE_CKPT="${BASELINE_CKPT:-/root/work/M3-ReID/ckptlog/BUPTCampus/Time-2026-05-24_21-40-49_SchemeAANoBoT_quad_calibrated_fromW5_fixdim_pa0.010_fa0.08_ca0.025_t10_buptcampus_v100_bs16/modelckpt/model_best.pth}"
WORKERS="${WORKERS:-2}"
GPU="${GPU:-0}"
...
PROJECTION_ALPHA="${PROJECTION_ALPHA:-0.015}"
CALIBRATION_ALPHA="${CALIBRATION_ALPHA:-0.025}"
PROJECTION_ALPHA_TARGET="${PROJECTION_ALPHA_TARGET:-${PROJECTION_ALPHA}}"
CALIBRATION_ALPHA_TARGET="${CALIBRATION_ALPHA_TARGET:-${CALIBRATION_ALPHA}}"
BRANCH_GATE_WEIGHT="${BRANCH_GATE_WEIGHT:-0.004}"
PART_LR_MULT="${PART_LR_MULT:-3.0}"
FREEZE_BASE_EPOCHS="${FREEZE_BASE_EPOCHS:-10}"
LR="${LR:-0.0000004}"
...
FUSION_ID_WEIGHT="${FUSION_ID_WEIGHT:-0.006}"
FUSION_TRIPLET_WEIGHT="${FUSION_TRIPLET_WEIGHT:-0.003}"
PROJECTION_ID_WEIGHT="${PROJECTION_ID_WEIGHT:-0.010}"
PROJECTION_TRIPLET_WEIGHT="${PROJECTION_TRIPLET_WEIGHT:-0.005}"
PROJECTION_MMA_WEIGHT="${PROJECTION_MMA_WEIGHT:-0.0005}"
PROJECTION_GATE_WEIGHT="${PROJECTION_GATE_WEIGHT:-0.003}"
PROJECTION_GATE_IR_TARGET="${PROJECTION_GATE_IR_TARGET:-0.60}"
PROJECTION_GATE_RGB_TARGET="${PROJECTION_GATE_RGB_TARGET:-0.40}"
CALIBRATION_ID_WEIGHT="${CALIBRATION_ID_WEIGHT:-0.016}"
CALIBRATION_TRIPLET_WEIGHT="${CALIBRATION_TRIPLET_WEIGHT:-0.008}"
CALIBRATION_MMA_WEIGHT="${CALIBRATION_MMA_WEIGHT:-0.001}"
CALIBRATION_GATE_WEIGHT="${CALIBRATION_GATE_WEIGHT:-0.004}"
...
DESC="${DESC:-SchemeAE2NoBoT_reliability_quad_fixloss_fromAA_fa${FUSION_ALPHA}_pa${PROJECTION_ALPHA}_ca${CALIBRATION_ALPHA}_bgw${BRANCH_GATE_WEIGHT}_t10_buptcampus_v100_bs$((P_NUM * K_NUM))}"
...
python train_m3reid.py \
  ...
  --use_m3plus \
  --m3plus_mode reliability_quad_calibrated_fusion \
  ...
  --fusion_id_weight "${FUSION_ID_WEIGHT}" \
  --fusion_triplet_weight "${FUSION_TRIPLET_WEIGHT}" \
  ...
  --projection_alpha_target "${PROJECTION_ALPHA_TARGET}" \
  --calibration_alpha_target "${CALIBRATION_ALPHA_TARGET}" \
  ...
  --gpu "${GPU}" \
  "${EXTRA_ARGS[@]}" \
  2>&1 | tee "${LOG_DIR}/scheme_ae2_t10_buptcampus_v100_$(date +%Y%m%d_%H%M%S).log"
