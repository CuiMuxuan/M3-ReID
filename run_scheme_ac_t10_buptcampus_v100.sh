...
CALIBRATION_ID_WEIGHT="${CALIBRATION_ID_WEIGHT:-0.024}"
CALIBRATION_TRIPLET_WEIGHT="${CALIBRATION_TRIPLET_WEIGHT:-0.012}"
CALIBRATION_MMA_WEIGHT="${CALIBRATION_MMA_WEIGHT:-0.0012}"
...
DESC="${DESC:-SchemeACNoBoT_direction_aware_quad_fromAA_fa${FUSION_ALPHA}_pa${PROJECTION_ALPHA}_ca${CALIBRATION_ALPHA}_rw${ROUTER_WEIGHT}_t10_buptcampus_v100_bs$((P_NUM * K_NUM))}"
...
python train_m3reid.py \
  ...
  --gpu "${GPU}" \
  "${EXTRA_ARGS[@]}" \
  2>&1 | tee "${LOG_DIR}/scheme_ac_t10_buptcampus_v100_$(date +%Y%m%d_%H%M%S).log"
