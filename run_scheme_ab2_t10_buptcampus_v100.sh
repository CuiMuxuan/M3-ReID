...
BRANCH_GATE_WEIGHT="${BRANCH_GATE_WEIGHT:-0.003}"
...
DESC="${DESC:-SchemeAB2NoBoT_adaptive_quad_fromAA_fa${FUSION_ALPHA}_pa${PROJECTION_ALPHA}_ca${CALIBRATION_ALPHA}_t10_buptcampus_v100_bs$((P_NUM * K_NUM))}"
...
python train_m3reid.py \
  ...
  --use_m3plus \
  --m3plus_mode adaptive_quad_calibrated_fusion \
  ...
  --gpu "${GPU}" \
  "${EXTRA_ARGS[@]}" \
  2>&1 | tee "${LOG_DIR}/scheme_ab2_t10_buptcampus_v100_$(date +%Y%m%d_%H%M%S).log"
