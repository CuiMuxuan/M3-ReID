...
PART_ID_WEIGHT="${PART_ID_WEIGHT:-0.06}"
PART_TRIPLET_WEIGHT="${PART_TRIPLET_WEIGHT:-0.03}"
PART_MMA_WEIGHT="${PART_MMA_WEIGHT:-0.004}"
PROJECTION_ID_WEIGHT="${PROJECTION_ID_WEIGHT:-0.035}"
PROJECTION_TRIPLET_WEIGHT="${PROJECTION_TRIPLET_WEIGHT:-0.018}"
PROJECTION_MMA_WEIGHT="${PROJECTION_MMA_WEIGHT:-0.0015}"
PROJECTION_GATE_WEIGHT="${PROJECTION_GATE_WEIGHT:-0.015}"
PROJECTION_GATE_IR_TARGET="${PROJECTION_GATE_IR_TARGET:-0.65}"
PROJECTION_GATE_RGB_TARGET="${PROJECTION_GATE_RGB_TARGET:-0.35}"
CALIBRATION_ID_WEIGHT="${CALIBRATION_ID_WEIGHT:-0.015}"
CALIBRATION_TRIPLET_WEIGHT="${CALIBRATION_TRIPLET_WEIGHT:-0.008}"
CALIBRATION_MMA_WEIGHT="${CALIBRATION_MMA_WEIGHT:-0.0008}"
...
DESC="${DESC:-SchemeXNoBoT_projection_calibrated_alpha${FUSION_ALPHA}_cal${CALIBRATION_ALPHA}_t10_buptcampus_v100_bs$((P_NUM * K_NUM))_from_nobot}"
...
python train_m3reid.py \
  ...
  --use_m3plus \
  --m3plus_mode projection_calibrated_fusion \
  ...
  --gpu "${GPU}" \
  "${EXTRA_ARGS[@]}" \
  2>&1 | tee "${LOG_DIR}/scheme_x_t10_buptcampus_v100_$(date +%Y%m%d_%H%M%S).log"
