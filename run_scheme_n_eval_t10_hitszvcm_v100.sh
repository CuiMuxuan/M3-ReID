#!/usr/bin/env bash
set -euo pipefail

export M3PLUS_MODE="${M3PLUS_MODE:-dual_fusion}"
export FUSION_ALPHA="${FUSION_ALPHA:-0.01}"
export PART_MATCH_WEIGHT="${PART_MATCH_WEIGHT:-0.0}"
export PART_MATCH_WEIGHT_I2V="${PART_MATCH_WEIGHT_I2V:-0.02}"
export PART_MATCH_WEIGHT_V2I="${PART_MATCH_WEIGHT_V2I:-0.0}"
export PART_MATCH_NEIGHBOR_RADIUS="${PART_MATCH_NEIGHBOR_RADIUS:-1}"
export PART_MATCH_SYMMETRIC="${PART_MATCH_SYMMETRIC:-1}"
export PART_RERANK_TOPK="${PART_RERANK_TOPK:-20}"
export PART_RERANK_TOPK_I2V="${PART_RERANK_TOPK_I2V:-10}"
export PART_RERANK_TOPK_V2I="${PART_RERANK_TOPK_V2I:-30}"
export PART_RERANK_WEIGHT="${PART_RERANK_WEIGHT:-0.0}"
export PART_RERANK_WEIGHT_I2V="${PART_RERANK_WEIGHT_I2V:-0.015}"
export PART_RERANK_WEIGHT_V2I="${PART_RERANK_WEIGHT_V2I:-0.015}"
export PART_RERANK_NORM="${PART_RERANK_NORM:-zscore}"
export RECIPROCAL_TOPK="${RECIPROCAL_TOPK:-20}"
export RECIPROCAL_WEIGHT="${RECIPROCAL_WEIGHT:-0.0}"
export RECIPROCAL_WEIGHT_I2V="${RECIPROCAL_WEIGHT_I2V:-0.022}"
export RECIPROCAL_WEIGHT_V2I="${RECIPROCAL_WEIGHT_V2I:-0.018}"
export DESC="${DESC:-SchemeN_reciprocal_schemeD_alpha${FUSION_ALPHA}_rk${RECIPROCAL_TOPK}_rw${RECIPROCAL_WEIGHT_I2V}_${RECIPROCAL_WEIGHT_V2I}_t10_hitszvcm_v100}"

bash ./run_scheme_h_eval_t10_hitszvcm_v100.sh
