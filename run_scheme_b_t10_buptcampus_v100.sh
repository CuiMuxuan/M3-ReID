...
DESC="${DESC:-SchemeB_baseline_warm_partonly_t10_buptcampus_v100_bs$((P_NUM * K_NUM))}"
...
python train_m3reid.py \
  ...
  --gpu "${GPU}" \
  "${EXTRA_ARGS[@]}" \
  2>&1 | tee "${LOG_DIR}/scheme_b_t10_buptcampus_v100_$(date +%Y%m%d_%H%M%S).log"
