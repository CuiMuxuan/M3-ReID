...
PROJECTION_ALPHA="${PROJECTION_ALPHA:-0.0}"
CALIBRATION_ALPHA="${CALIBRATION_ALPHA:-0.0}"
PART_LR_MULT="${PART_LR_MULT:-5.0}"
FREEZE_BASE_EPOCHS="${FREEZE_BASE_EPOCHS:-8}"
LR="${LR:-0.000001}"
TRIPLET_WEIGHT="${TRIPLET_WEIGHT:-0.04}"
TRIPLET_FRAME_WEIGHT="${TRIPLET_FRAME_WEIGHT:-0.08}"
PART_ID_WEIGHT="${PART_ID_WEIGHT:-0.18}"
PART_TRIPLET_WEIGHT="${PART_TRIPLET_WEIGHT:-0.12}"
PART_MMA_WEIGHT="${PART_MMA_WEIGHT:-0.015}"
FUSION_ID_WEIGHT="${FUSION_ID_WEIGHT:-0.004}"
FUSION_TRIPLET_WEIGHT="${FUSION_TRIPLET_WEIGHT:-0.002}"
PROJECTION_ID_WEIGHT="${PROJECTION_ID_WEIGHT:-0.004}"
PROJECTION_TRIPLET_WEIGHT="${PROJECTION_TRIPLET_WEIGHT:-0.002}"
PROJECTION_MMA_WEIGHT="${PROJECTION_MMA_WEIGHT:-0.0002}"
PROJECTION_GATE_WEIGHT="${PROJECTION_GATE_WEIGHT:-0.0}"
PROJECTION_GATE_IR_TARGET="${PROJECTION_GATE_IR_TARGET:-0.65}"
PROJECTION_GATE_RGB_TARGET="${PROJECTION_GATE_RGB_TARGET:-0.35}"
CALIBRATION_ID_WEIGHT="${CALIBRATION_ID_WEIGHT:-0.004}"
CALIBRATION_TRIPLET_WEIGHT="${CALIBRATION_TRIPLET_WEIGHT:-0.002}"
CALIBRATION_MMA_WEIGHT="${CALIBRATION_MMA_WEIGHT:-0.0002}"
CALIBRATION_GATE_WEIGHT="${CALIBRATION_GATE_WEIGHT:-0.0}"
CALIBRATION_GATE_TARGET="${CALIBRATION_GATE_TARGET:-0.60}"
BRANCH_GATE_WEIGHT="${BRANCH_GATE_WEIGHT:-0.002}"
...
LR_MILESTONES="${LR_MILESTONES:-20,25}"
DESC="${DESC:-SchemeAJNoBoT_reliability_anchor_quad_fromD_pa${PROJECTION_ALPHA}_fa${FUSION_ALPHA}_ca${CALIBRATION_ALPHA}_t10_hitszvcm_v100_bs$((P_NUM * K_NUM))}"
...
python train_m3reid.py \
  ...
  --use_m3plus \
  --m3plus_mode reliability_anchor_quad_fusion \
  ...
  --projection_alpha_target "${PROJECTION_ALPHA}" \
  --calibration_alpha_target "${CALIBRATION_ALPHA}" \
  ...
  --gpu "${GPU}" \
  "${EXTRA_ARGS[@]}" \
  2>&1 | tee "${LOG_DIR}/scheme_aj_t10_hitszvcm_v100_$(date +%Y%m%d_%H%M%S).log"
