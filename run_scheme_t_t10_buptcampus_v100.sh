...
ROUTER_WEIGHT="${ROUTER_WEIGHT:-0.05}"
...
DESC="${DESC:-SchemeTNoBoT_bidirectional_calibration_t10_buptcampus_v100_bs$((P_NUM * K_NUM))_from_nobot}"
...
python train_m3reid.py \
  ...
  --use_m3plus \
  --m3plus_mode bidirectional_calibration \
  ...
  --gpu "${GPU}" \
  "${EXTRA_ARGS[@]}" \
  2>&1 | tee "${LOG_DIR}/scheme_t_t10_buptcampus_v100_$(date +%Y%m%d_%H%M%S).log"
