...
FUSION_ALPHA="${FUSION_ALPHA:-0.10}"
CALIBRATION_ALPHA="${CALIBRATION_ALPHA:-0.01}"
PART_LR_MULT="${PART_LR_MULT:-4.0}"
TEMPORAL_LR_MULT="${TEMPORAL_LR_MULT:-4.0}"
...
DESC="${DESC:-SchemeZNoBoT_temporal_dual_calibrated_alpha${FUSION_ALPHA}_cal${CALIBRATION_ALPHA}_t10_buptcampus_v100_bs$((P_NUM * K_NUM))_from_nobot}"
...
python train_m3reid.py \
  ...
  --use_m3plus \
  --m3plus_mode temporal_dual_calibrated_fusion \
  ...
  --gpu "${GPU}" \
  "${EXTRA_ARGS[@]}" \
  2>&1 | tee "${LOG_DIR}/scheme_z_t10_buptcampus_v100_$(date +%Y%m%d_%H%M%S).log"
