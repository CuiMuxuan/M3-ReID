...
LR_MILESTONES="${LR_MILESTONES:-30,40}"
DESC="${DESC:-SchemeQ_part_token_fusion_fromDv1_t10_hitszvcm_v100_bs$((P_NUM * K_NUM))}"
...
python train_m3reid.py \
  ...
  --gpu "${GPU}" \
  "${EXTRA_ARGS[@]}" \
  2>&1 | tee "${LOG_DIR}/scheme_q_t10_hitszvcm_v100_$(date +%Y%m%d_%H%M%S).log"
