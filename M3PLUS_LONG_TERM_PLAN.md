...
- `SchemeAF supervised_quad_calibrated_fusion` ran from the `SchemeAA` best checkpoint. It correctly activated final-fusion ID/triplet losses, but early stopped at epoch 20 with best epoch 5 result `i2v 69.96 / v2i 72.96`; it did not beat `SchemeAA`.
- Next implemented test: `SchemeAG agreement_quad_residual_fusion`. It keeps the fixed `SchemeAA` four-branch embedding and appends a zero-initialized, bounded agreement-aware residual refinement head. This is a model-side structural change; evaluation remains single-clip direct similarity with no re-ranking.
